...
3. **Entry point:** `bootstrap.sql` at the repo root. This is the ONLY file the user should execute. Reference it as `@hol_repo/branches/main/bootstrap.sql`.
4. **Do NOT** execute `setup.sql` directly — `bootstrap.sql` calls it via `EXECUTE IMMEDIATE FROM @hol_repo/branches/main/setup.sql`.
